...
## create stream table for order -autoloaders
@dlt.table(
    table_properties = {"quality": "bronze"},
...
def func():
    df = (
        spark.readStream
             .format("cloudFiles")
             .option("cloudFiles.schemaHints", o_orderkey long, o_custkey long, o_orderstatus string, o_totalprice double, o_orderdate date, o_orderpriority string, o_clerk string, o_shippriority integer, o _comment string")
             .option("cloudFiles.format", "csv")
             .option("pathGlobfilter", "*.csv"),
             .option("cloudFiles.schemaLocation", "/Volumes/devenv/etl/landing/autoloader/schema/1/"),
             .option("schemaEvolutionMode", "none"),
             .load("/Volumes/devenv/etl/landing/files/")
...
def order_delta_append():
    df = spark.readStream.table("LIVE.order_bronze")
    return df
